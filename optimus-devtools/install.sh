...
output $OUTPUT_MODE
output $OUTPUT_MODE "INSTALLATION DES OUTILS DE DEVELOPPEMENT" "blue" 200 "dev-tools" 0


output $OUTPUT_MODE "Activation de la connexion à distance sur le port 3306 pour l'utilisateur root" "magenta" 200 "dev-tools" 10
if [ $(which /sbin/ufw) ]
then 
...
output $OUTPUT_MODE "Installation des dépôts complémentaires" "green" 200 "dev-tools" 20
...
output $OUTPUT_MODE "Ajout de l'utilisateur debian au groupe www-data" "magenta" 200 "dev-tools" 30
verbose usermod -a -G www-data debian


#OPTIMUS-BASE
output $OUTPUT_MODE "Installation du conteneur optimus-base" "green" 200 "dev-tools" 40
if [ $( docker ps -a | grep optimus-base | wc -l ) -gt 0 ]
then
	output $OUTPUT_MODE "Suppression du conteneur optimus-base existant" "magenta" 200 "dev-tools" 45
...
output $OUTPUT_MODE "Téléchargement de l'image optimus-base" "magenta" 200 "dev-tools" 50
verbose docker pull --quiet git.cybertron.fr:5050/optimus/optimus-base/v5:latest

output $OUTPUT_MODE "Création du conteneur optimus-base (développement)" "magenta" 200 "dev-tools" 55
...
output $OUTPUT_MODE "Lancement du conteneur optimus-base (développement)" "magenta" 200 "dev-tools" 60
verbose docker start optimus-base


#OPTIMUS-DEVTOOLS
output $OUTPUT_MODE "Installation du conteneur optimus-devtools" "green" 200 "dev-tools" 70
if [ $( docker ps -a | grep optimus-devtools | wc -l ) -gt 0 ]
then
	output $OUTPUT_MODE "Suppression du conteneur optimus-devtools existant" "magenta" 200 "dev-tools" 75
...
output $OUTPUT_MODE "Téléchargement de l'image optimus-devtools" "magenta" 200 "dev-tools" 80
verbose docker pull --quiet git.cybertron.fr:5050/optimus/optimus-devtools/v5:latest

output $OUTPUT_MODE "Création du conteneur optimus-devtools (développement)" "magenta" 200 "dev-tools" 85
verbose docker create \
...
--env KEEP_ROOT_ACCESS=1 \og/optimus \
--volume /srv/vhosts:/srv/vhosts \
--volume /srv/optimus:/srv/optimus \
--volume /srv/services:/srv/services \
...
output $OUTPUT_MODE "Lancement du conteneur optimus-devtools (développement)" "magenta" 200 "dev-tools" 90
verbose docker start optimus-devtools

output $OUTPUT_MODE "Attribution des droits en écriture sur les dépôts" "magenta" 200 "dev-tools" 95
verbose chmod 775 -R /srv/optimus
verbose chown www-data:www-data -R /srv/optimus

output $OUTPUT_MODE "Les outils de développement ont été installés avec succès" "green" 200 "dev-tools" 100
