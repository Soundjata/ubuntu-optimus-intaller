...
output $OUTPUT_MODE "Création du groupe mysql (gid 221)" "magenta" 200 "optimus-databases" 10
verbose groupadd --gid 221 mysql

output $OUTPUT_MODE "Création de l'utilisateur mysql (uid 221)" "magenta" 200 "optimus-databases" 20
verbose useradd --no-create-home --home /nonexistent --shell /bin/false --gid 221 --uid 221 mysql 

if [ ! -d "/srv/databases" ]
then
	output $OUTPUT_MODE "Création du dossier /srv/databases" "magenta" 200 "optimus-databases" 30
...
then
	output $OUTPUT_MODE "Suppression du conteneur existant" "magenta" 200 "optimus-databases" 40
	verbose docker stop optimus-databases
	verbose docker rm optimus-databases
fi

output $OUTPUT_MODE "Téléchargement de l'image" "magenta" 200 "optimus-databases" 50
verbose docker pull --quiet git.cybertron.fr:5050/optimus/optimus-databases/v5:latest

output $OUTPUT_MODE "Création du conteneur" "magenta" 200 "optimus-databases" 60
...
output $OUTPUT_MODE "Lancement du conteneur" "magenta" 200 "optimus-databases" 70
verbose docker start optimus-databases

output $OUTPUT_MODE "Attente de l'ouverture du socket MARIADB" "magenta" 200 "optimus-databases" 80
WAITER=0
